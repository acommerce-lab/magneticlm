...
def build_spectrum(ctx_rows, ctx_cols, ctx_counts, unigram,
    ...
    # Build multiple relation matrices — normalize each independently
    TE = torch.sparse.mm(bg_trans.cpu(), embeddings)
    M_fwd = embeddings.T @ TE
    M_fwd = M_fwd / M_fwd.norm().clamp(min=1e-9)

    M_sim = embeddings.T @ embeddings
    M_sim = M_sim / M_sim.norm().clamp(min=1e-9)

    TE_bwd = torch.sparse.mm(bg_trans.t().cpu(), embeddings)
    M_bwd = embeddings.T @ TE_bwd
    M_bwd = M_bwd / M_bwd.norm().clamp(min=1e-9)

    # Equal-weight combination (each matrix contributes equally)
    M_total = M_fwd + M_sim + M_bwd
    U_m, S_m, Vh_m = torch.linalg.svd(M_total)

    # n_heads: find best divisor of d, minimum 2
    n_heads = 1
    for h in [2, 3, 4, 6, 7, 8, 12, 16, d]:
        if d % h == 0 and h <= d // 2:
            n_heads = h
    if n_heads < 2 and d >= 4:
        for h in [2, 3, 4, 6, 7, 8]:
            if d % h == 0: n_heads = h; break
    ...
    print(f"    d={d}, n_heads={n_heads}, d_head={d//n_heads}, built in {time.time()-t0:.1f}s")
...
def train_statistical(model, bigram_trans, V, context_len,
    ...
    """Train by sampling from bigram transition matrix (sparse, any V)."""
    device = next(model.parameters()).device

    # Extract sparse pairs: (prev_word, next_word, probability)
    bg_cpu = bigram_trans.coalesce()
    indices = bg_cpu.indices()  # [2, E]
    values = bg_cpu.values()    # [E]
    prev_words = indices[0].to(device)
    next_words = indices[1].to(device)
    probs = values.to(device)
    E = prev_words.shape[0]
    print(f"    Statistical training: {E:,} bigram pairs, V={V}")
    ...
    for epoch in range(max_epochs):
        ...
        for start in range(0, E, batch_size):
            ...
            idx = perm[start:end]
            pw = prev_words[idx].unsqueeze(1).long()  # [batch, 1]
            nw = next_words[idx].long()  # [batch]

            optimizer.zero_grad()
            logits = raw_model(pw, torch.ones_like(pw, dtype=torch.bool))
            loss = F.cross_entropy(logits, nw)
            loss.backward()
            optimizer.step()
            total_loss += loss.detach().cpu().item()
            n_batches += 1
        ...
        if val_loader:
            ...
            with torch.no_grad():
                ...
                for ctx_b, tgt_b in val_loader:
                    ctx_b, tgt_b = ctx_b.to(device), tgt_b.to(device)
                    logits = raw_model(ctx_b, ctx_b > 0)
                    vl += F.cross_entropy(logits, tgt_b.long()).detach().cpu().item() * ctx_b.shape[0]
                    vn += ctx_b.shape[0]
                val_ppl = math.exp(vl / max(vn, 1))

        tag = "↓" if val_ppl < best_val else "↑"
        print(f"    epoch {epoch}: train={train_ppl:.1f} val={val_ppl:.1f} {tag} ({elapsed:.1f}s)")
