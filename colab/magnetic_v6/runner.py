"""v6 Pipeline: stats → embeddings → KN → attention → eval."""
...
def run_pipeline(cfg: Config) -> Dict:
    ...
    print("=" * 72)
    print("MagneticLM v6 — Deterministic Attention")
    print("=" * 72)
    ...
    results["layers"] = eval_layers(kn, embeddings, idf, enc_valid, cfg, res.primary_device)

    if cfg.eval_ood_cloze:
        print("  [eval] OOD cloze...")
        results["ood"] = eval_ood(kn, embeddings, idf, vocab, cfg, res.primary_device)

    print(f"  Total eval: {time.time()-t_eval:.1f}s")
    mon.snapshot("post-eval")

    os.makedirs(cfg.save_dir, exist_ok=True)
    out = os.path.join(cfg.save_dir, "v6_results.json")
